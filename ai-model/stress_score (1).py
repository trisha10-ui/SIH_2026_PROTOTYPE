...
# ---- STEP 3: Function to decide the stress level from the score ----
...
# ---- STEP 4: Check if a line should be skipped (headings/keywords/blank) ----
...
def main():
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            results.append((message, score, level))

    with open(output_file, "w", encoding="utf-8") as f:
        for message, score, level in results:
            f.write(f"Message: {message}\n")
            f.write(f"Score: {score}/10\n")
            f.write(f"Stress Level: {level}\n")
            f.write("-" * 50 + "\n")

    print(f"Done! Processed {len(results)} messages.")
